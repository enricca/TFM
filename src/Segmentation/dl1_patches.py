...
def compute_ROIs(generate_csv=False,
    ...
    patients_with_annotations = pd.read_csv(
        NODULES_PATH)  # filter patients with no annotations to avoid having to read them
    ...
    filenames = [os.path.join(INPUT_PATH, fp) for fp in filenames if
                 fp in patients_with_annotations or not filter_annotated]
    filenames = filter(os.path.isfile, filenames)

    def __load_and_store(filename):
        patient_data = np.load(filename)['arr_0']
        patient_data = patient_data.astype(np.int16)
        X, y, rois, stats = common.load_patient(patient_data, discard_empty_nodules=True, output_rois=True, debug=True,
                                                include_ground_truth=True, thickness=1)
        if not stats:
...
def train(load_model=False, model='patches', version=0):
    ...
    model.fit_generator(generator=chunks(x_train, y_train, batch_size=32, thickness=1, data_generator=train_datagen),
                        ...
                        callbacks=[tb, model_checkpoint, roc_callback(
                            chunks(x_test, y_test, batch_size=500, thickness=1, data_generator=test_datagen,
                                   is_training=False, ))],
                        validation_data=chunks(x_test, y_test, batch_size=32, thickness=1, data_generator=test_datagen,
                                               is_training=False, ),
...
OUTPUT_MODEL = od + 'models/%s_v%s.hdf5' % (
args.network, args.version)  # OUTPUT_MODEL = wp + 'personal/jm_patches_train_v06_local.hdf5'
