...
def process_filenames_sequencial(filenames_list, model_path, output_csv, nodules_df=None):
    ...
    for filename in filenames_list:
        try:    
            ...
            logging.info("[Process Sequencial] Predicted patient %d %s. Batch results: %d/%d (th=0.7)" % (total, filename, len([p for p in preds if p>0.7]),len(preds)))
...
def evaluate_model(file_list, model_path, output_csv, nodules_df=None, parallel = False):
    
    if parallel:
        manager = multiprocessing.Manager()
        q = manager.Queue()
        pool = multiprocessing.Pool(5, maxtasksperchild = 1)  # multiprocessing.cpu_count()
        ...
        process_filenames_sequencial(file_list, model_path, output_csv, nodules_df)
...
if __name__ == "__main__":
    ...
    # DEFAULT VALUES
    wp = '/media/shared/datasets/LUNA/'  #wp = os.environ['LUNG_PATH']
    INPUT_PATH = wp + 'preprocessed' #INPUT_PATH = '/mnt/hd2/preprocessed5'  # INPUT_PATH = wp + 'data/preprocessed5_sample'
    MODEL = wp + 'models/patches_v1.hdf5' #MODEL = wp + 'models/jm_patches_train_v19.hdf5'
    OUTPUT_CSV = wp + 'output/nodules_patches_dl1_v1_luna.csv' #OUTPUT_CSV = wp + 'output/nodules_patches_dl1_v19_luna.csv'
