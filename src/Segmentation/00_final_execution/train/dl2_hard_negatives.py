...
LUNA_ANNOTATIONS = wp + 'CSVFILES/annotations.csv'
OUTPUT_DL1 = wp + 'output/nodules_patches_dl1_v1_luna.csv'  # OUTPUT_DL1 = wp + 'personal/noduls_patches_v06.csv'
INPUT_PATH = wp + 'preprocessed/'  # INPUT_PATH = wp + 'data/preprocessed5_sample'
VALIDATION_PATH = wp + 'preprocessed_validation_luna/' # VALIDATION_PATH = wp + 'data/preprocessed5_sample'
PATCHES_PATH = wp + 'patches'  # PATCHES_PATH = wp + 'data/preprocessed5_patches'

OUTPUT_MODEL = wp + 'models/patches_hard_negatives_v1.hdf5'  # OUTPUT_MODEL = wp + 'personal/jm_patches_train_v06_local.hdf5'
LOGS_PATH = wp + 'logs/%s' % 'hard_negatives_v1' #str(int(time()))
...
common.multiproc_crop_generator(filenames_train,
                                os.path.join(PATCHES_PATH,'dl2_v1_x_train.npz'),
                                os.path.join(PATCHES_PATH,'dl2_v1_y_train.npz'),
                                __load_and_store,
                                parallel=True)


common.multiproc_crop_generator(filenames_test,
                                os.path.join(PATCHES_PATH,'dl2_v1_x_test.npz'),
                                os.path.join(PATCHES_PATH,'dl2_v1_y_test.npz'),
                                __load_and_store,
                                parallel=True)
...
x_train = np.load(os.path.join(PATCHES_PATH, 'dl2_v1_x_train.npz'))['arr_0']
y_train = np.load(os.path.join(PATCHES_PATH, 'dl2_v1_y_train.npz'))['arr_0']
x_test = np.load(os.path.join(PATCHES_PATH, 'dl2_v1_x_test.npz'))['arr_0']
y_test = np.load(os.path.join(PATCHES_PATH, 'dl2_v1_y_test.npz'))['arr_0']
