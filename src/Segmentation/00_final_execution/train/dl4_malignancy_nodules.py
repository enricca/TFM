...
def compute_ROIs(generate_csv=False, 
                 ...
                 mode = 'train'):
    if generate_csv:
        with open(os.path.join(OUTPUT_CSV,'ROIs_dl4_v{}_{}_nodules.csv'.format(version, mode)), 'w') as f:
    ...
    def __load_and_store(filename):
        ...
        if generate_csv:
            with open(os.path.join(OUTPUT_CSV,'ROIs_dl4_v{}_{}_nodules.csv'.format(version, mode)), 'a') as f:
    ...
    common.multiproc_crop_generator(filenames_train,
                                    os.path.join(PATCHES_PATH, 'dl4_v{}_x_{}_nodules.npz'.format(version, mode)),
                                    os.path.join(PATCHES_PATH, 'dl4_v{}_y_{}_nodules.npz'.format(version, mode)),
                                    __load_and_store)
...
def train(load_model=False, version = 0):
    ...
    x_test = np.load(os.path.join(PATCHES_PATH, 'dl4_v{}_x_test_nodules.npz'.format(version)))['arr_0']
    y_test = np.load(os.path.join(PATCHES_PATH, 'dl4_v{}_y_test_nodules.npz'.format(version)))['arr_0']
    y_test = y_test/84.
    y_test[y_test < 0] = -1
    ...
    x_train = np.load(os.path.join(PATCHES_PATH, 'dl4_v{}_x_train_nodules.npz'.format(version)))['arr_0']
    y_train = np.load(os.path.join(PATCHES_PATH, 'dl4_v{}_y_train_nodules.npz'.format(version)))['arr_0']
    y_train = y_train/84.
    y_train[y_train < 0] = -1
...
# ARGUMENTS    
parser = argparse.ArgumentParser(description='Train dl4 nodules')
...
parser.add_argument('--network', default = 'basic', help='[NOT USED] basic network /hard negatives / malignacy / nodules ')
...
OUTPUT_MODEL = od + 'models/malignancy_nodules_v{}.hdf5'.format(args.version)  # OUTPUT_MODEL = wp + 'personal/jm_patches_train_v06_local.hdf5'
LOGS_PATH = od + 'logs/%s' % 'malignancy_nodules_v{}'.format(args.version)
