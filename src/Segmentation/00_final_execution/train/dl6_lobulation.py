...
def compute_ROIs(generate_csv=False, 
                 ...
                 mode = 'train'):
    if generate_csv:
        with open(os.path.join(OUTPUT_CSV,'ROIs_dl6_v{}_{}.csv'.format(version, mode)), 'w') as f:
    ...
    def __load_and_store(filename):
        ...
        if generate_csv:
            with open(os.path.join(OUTPUT_CSV,'ROIs_dl6_v{}_{}.csv'.format(version, mode)), 'a') as f:
    ...
    common.multiproc_crop_generator(filenames_train,
                                    os.path.join(PATCHES_PATH, 'dl6_v{}_x_{}.npz'.format(version, mode)),
                                    os.path.join(PATCHES_PATH, 'dl6_v{}_y_{}.npz'.format(version, mode)),
                                    __load_and_store)
...
def train(load_model=False, version = 0):
    ...
    x_test = np.load(os.path.join(PATCHES_PATH, 'dl6_v{}_x_test.npz'.format(version)))['arr_0']
    y_test = np.load(os.path.join(PATCHES_PATH, 'dl6_v{}_y_test.npz'.format(version)))['arr_0']
    y_test = y_test/84.
    y_test[y_test < 0] = -1
...
# ARGUMENTS    
parser = argparse.ArgumentParser(description='Train dl6')
...
OUTPUT_MODEL = od + 'models/lobulation_v{}.hdf5'.format(args.version)  # OUTPUT_MODEL = wp + 'personal/jm_patches_train_v06_local.hdf5'
LOGS_PATH = od + 'logs/%s' % 'lobulation_v{}'.format(args.version)
