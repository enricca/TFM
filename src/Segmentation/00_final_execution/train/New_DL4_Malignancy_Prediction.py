...
import numpy as np
import pandas as pd
#from dl_model_patches import common <-- Substutite line below with this once commons are merged
import common
from keras import backend as K
from keras.preprocessing.image import ImageDataGenerator
from keras.optimizers import Adam
from keras.callbacks import ModelCheckpoint, Callback
from dl_networks.sample_resnet_malignancy import ResnetBuilder
...
def compute_ROIs(generate_csv=False, 
    ...
    def __load_and_store(filename):
        ...
        X, y, rois, stats = common.load_patient_malignancy(patient_data, discard_empty_nodules=True, output_rois=True, debug=True, include_ground_truth=True, thickness=1)
        if not stats:
...
def train(load_model=False, version = 0):
    ...
    #Custom metric for the model (R2)
    ...
    model.compile(optimizer=Adam(lr=1e-4), loss='mean_squared_error', metrics=[R2])
...
##OUTPUT_MODEL = od + 'models/patches_v{}.hdf5'.format(args.version)  # OUTPUT_MODEL = wp + 'personal/jm_patches_train_v06_local.hdf5'
##LOGS_PATH = od + 'logs/%s' % 'patches_v{}'.format(args.version)
...
wp = '/home/pau/data2/'
od = '/home/pau/'
INPUT_PATH = wp # INPUT_PATH = wp + 'data/preprocessed5_sample'
# VALIDATION_PATH = wp + 'preprocessed_validation_luna'
NODULES_PATH = '/media/shared/datasets/LUNA/CSVFILES/annotations.csv'
PATCHES_PATH = wp + 'patches'  # PATCHES_PATH = wp + 'data/preprocessed5_patches'
OUTPUT_MODEL = od + 'models/patches_v{}.hdf5'.format(args.version)  # OUTPUT_MODEL = wp + 'personal/jm_patches_train_v06_local.hdf5'
LOGS_PATH = od + 'logs/%s' % 'patches_v{}'.format(args.version)
if not os.path.exists(LOGS_PATH):
