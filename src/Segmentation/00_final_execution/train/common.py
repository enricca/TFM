    ...
    for nslice in nslices:
        ...
        if patient_nodules_df is None:
            ...
            if lung_volume_l < 0.009 or lung_volume_l > 0.1:
                logging.info('Ignoring slice %d.' % nslice)
...
def get_labels_from_regions_malignancy(regions_real,malign_mask):   
    labels = [0]*len(regions_real)
    for idx,region_real in enumerate(regions_real):
        if region_real.bbox[0]==0:
            continue
        labels[idx] = malign_mask[np.int(region_real.centroid[0])][np.int(region_real.centroid[1])]
    return labels

def load_patient_malignancy(patient_data, discard_empty_nodules=False, thickness=0):
    """
    Returns images generated for each patient.
     - thickness: number of slices up and down to be taken
     - include_ground_truth: should the ground truth patches be included as regions?
    ...
    # Check if it has nodules annotated it should, else this function has no sense.
    if (patient_data.shape[0]!=3 and patient_data.shape[0]!=4):
        aux = np.zeros((4,patient_data.shape[1], patient_data.shape[2], patient_data.shape[3]))
        aux[0] = patient_data[0]
        aux[1] = patient_data[1]
        patient_data = aux
    ...
    for nslice in nslices:
        lung_image, lung_mask, nodules_mask, malign_mask = patient_data[0,nslice,:,:], patient_data[1,nslice,:,:], patient_data[2,nslice,:,:],patient_data[3,nslice,:,:]
...
#             logging.info('Ignoring slice %d.' % nslice)
        ...
        # Generate labels
        regions_real = get_regions(nodules_mask, threshold=np.mean(nodules_mask))
        labels = get_labels_from_regions_malignancy(regions_real,malign_mask)
        ...
        if thickness>0:  # add extra images as channels for thick resnet
            ...
            if lung_image.shape[0] != 2*thickness + 1:  # skip the extremes
                continue
        cropped_images = extract_crops_from_regions(lung_image, regions_real)
