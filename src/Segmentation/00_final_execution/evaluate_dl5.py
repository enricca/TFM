...
if __name__ == '__main__':
    ...
    parser.add_argument('-output_dl4', help='path of the output csv of dl4')
    ...
    parser.add_argument('--eval_all',  action='store_true', help='Evals all rois from dl4.')    
    ...
    SCORE_TH = 0.5
    nodules_df = pd.read_csv(args.output_dl4)
...
#         nodules_df = nodules_df[(nodules_df['score'] > SCORE_TH) | (nodules_df['label']==1)]
    ...
    for pN, patientPath in enumerate(patientFiles):
        try:
            ...
            X, y, rois, statsROIGeneration = common.load_patient(patient_data, discard_empty_nodules=False, output_rois=True, thickness=1, convertToFloat = args.convertToFloat, malignancy=True)
